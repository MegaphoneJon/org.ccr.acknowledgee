...
<style>
  .is_recur-section,
  .is_recur-section label {font-weight:bold!important;}
</style>
{/literal}
...
<script type="text/javascript">
  ...
  function enableAcknowledgee( ) {
    var selectedValue = cj('input[name="soft_credit_type_id"]:checked'); 
    console.log(selectedValue.val());
    if ( selectedValue.val() == 2) {
      cj('.acknowledgee-section').show();
      cj('#honorType > .form-item').each(function() {
        cj(this).hide();
      });
      cj('#honorType > #editrow-first_name').show();
      cj('#honorType > #editrow-last_name').show();
    }
    else {
      cj('.acknowledgee-section').hide();
      cj('#honorType > .form-item').each(function() {
        cj(this).show();
      });
    }
  }
  cj('input[name="soft_credit_type_id"]').each(function() {
